...
//Escribi esto por amor por si no vuelvo jamas.
import java.util.Optional;

import structures.lists.Fifo;
import structures.lists.Sort;
import structures.tree.Node;
import structures.tree.Tree;

/**
 * Ofrece una base tecnica para trabajar con busquedas NO INFORMADAS, en
 * alguna estructura que implemente la clase Nodo como modelador de estados.
 * 
...
public interface Context
{
	...
	for (;;)// Ciclo Infinito
	{
		...
		else
		{
		    ...
		    if (k != null) if (n.getCost() < k.getCost())// si el costo del estado que ya tenemos es menor al
								 // explorado
		    {
     ...
     * @param eu es el limite de busqueda
     * 
     * @return Optional con el nodo solucion si existe
     */
     ...
     * mientras forma otro arbol solucion, NO ALTERA EL ESTADO
     * ORIGINAL DEL ARBOL
     * 
     * 
     * @param goal
    ...
    {
	var n1 = new Node(goal.state(), goal.getCost());// Se crea el estado padre de la solucion.
	Tree copy = new Tree(n1, "N", "Solucion b");// Creamos el arbol con el nodo final del original como origen.
	...
	while (!false)
	{
	    ...
	    node = new Node(node.state(), node.getCost());// Creamos el nuevo estado apartir del actual
